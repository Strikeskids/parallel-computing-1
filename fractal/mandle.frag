...
double sqmag(dvec2 v) {
	return v.x*v.x + v.y*v.y;
}

void main() {
	dvec2 complexPosition = (dvec2(windowPosition) - dvec2(0.5, 0.5)) * zoomRect.zw + zoomRect.xy;
	...
	if (iter == iterations) {
		...
		float val = clamp(iter + 1 - log(log(sqrt(float(sqmag(z))))) / log(2.0f), 2, iterations);
		float prod = 3;
